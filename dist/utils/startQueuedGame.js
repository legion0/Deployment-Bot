...
export const startQueuedGame = async (deploymentTime) => {
    ...
    const loggingChannel = await client.channels.fetch(config.loggingChannel).catch(() => null);
    if (hosts.length < 1 || players.length < 3) {
        ...
        for (const group of hostPlayerGroups) {
            ...
            const hostMember = await departureChannel.guild.members.fetch(host.user).catch(() => null);
...
async function updateQueueMessages(notEnoughPlayers = false, nextDeploymentTime, deploymentCreated = false) {
