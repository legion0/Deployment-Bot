 ...
 */
export function buildEmbed({ name, preset, embed, placeholders }) {
    if (!embed)
        embed = new EmbedBuilder();
    if (!name && !preset)
        throw new Error("You must provide a name or a preset to build an embed");
    const { embeds } = config;
    ...
    if (embeds[name]?.timestamp != undefined ? embeds[name]?.timestamp : presets[preset]?.timestamp != undefined ? presets[preset]?.timestamp : defaultPreset.timestamp)
        embed.setTimestamp();
    return embed;
}
/**
