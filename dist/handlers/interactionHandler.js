...
async function register(dir) {
    ...
    for (const file of files) {
        if (file.endsWith(".js") || file.endsWith(".ts")) {
            ...
            else if (dir.endsWith("buttons") || dir.endsWith("modals") || dir.endsWith("selectMenus"))
                identifier = interaction.id;
            if (!identifier)
                throw new Error(`No name or id found for ${dir}/${file}. Did I maybe mess up?`);
                ...
                directories.forEach(async (file2) => {
                    if (file2.endsWith(".js") || file2.endsWith(".ts")) {
                        ...
                        if (dir.endsWith("commands") || dir.endsWith("slashCommands") || dir.endsWith("contextMenus"))
                            ...
                            identifier = interaction.id;
                        if (!identifier)
                            throw new Error(`No name or id found for ${dir}/${file}/${file2}. Did I maybe mess up?`);
                        client[dirName].set(identifier, interaction);
                    }
                });
                continue;
