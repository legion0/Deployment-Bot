...
export async function discordClientReadyCallback(client: Client) {
	try {
		...
		await checkDeployments(client);
		cron.schedule('* * * * *', checkDeployments.bind(null, client));
...
async function sendDeploymentNotices(client: Client, now: DateTime) {
...
async function startDeployments(client: Client, now: DateTime) {
...
async function deleteOldDeployments(client: Client, now: DateTime) {
...
async function checkDeployments(client: Client) {
...
async function removeOldSignups() {
