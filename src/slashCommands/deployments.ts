...
const buildDeploymentsEmbed = async (user: string): Promise<EmbedBuilder> => {
    ...
    deployments.forEach(deployment => {
       ...
       // const link = `https://discord.com/channels/${Config.guildId}/1299122351291629599/${deployment.id}`;

        // Add field to the embed
        embed.addFields({
            name: '',
            value: `${getStatusEmoji(assignment)} ᲼**Operation:** ${title} | **Time:** <t:${Math.round(time / 1000)}:t> | **Assignment:** ${assignment.charAt(0).toUpperCase() + assignment.slice(1)} | [🔗]`
        });
    });
