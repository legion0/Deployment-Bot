...
export default new Button({
    ...
    func: async function ({ interaction }) {
        ...
        if (playersInQueue.length >= config.queueMaxes.players && !getHotDropQueue().strikeModeEnabled) {
            const errorEmbed = buildEmbed({ preset: "error" })
        ...
        try {
            ...
            await getHotDropQueue().updateMessage(/*notEnoughPlayers=*/true, /*deploymentCreated=*/false);
        } catch (error) {
