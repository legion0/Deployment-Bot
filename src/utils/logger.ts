...
export function log(text: string, context?: string) {
	...
	const contextStr = context ? colors.cyan(`[${context}]`) : '';
...
export function success(text: string, context?: string) {
	const date = new Date().toTimeString().split(/ +/)[0];
	const contextStr = context ? colors.cyan(`[${context}]`) : '';
	console.log(colors.rainbow(`[${date}]${contextStr}:  ${text}`));
}
...
export function error(text: String | Error, context?: string) {
	...
	if (config.debugMode) {
		if (text instanceof Error) {
			console.log(colors.red.bold(`[${date}]${contextStr}: Error: ${text.message}`));
			console.error(colors.red(text.stack));
		} else {
			console.log(colors.red.bold(`[${date}]${contextStr}: ${text}`));
		}
	} else {
		console.log(colors.red.bold(`[${date}]${contextStr}: ${text}`));
	}
